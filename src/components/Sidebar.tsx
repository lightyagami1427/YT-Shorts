  ...
  return (
    <aside className="sidebar flex flex-col bg-white overflow-y-auto border-r border-[#eaedf3] shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
      <div className="p-10 pb-6">
        <h1 className="text-3xl font-black text-[#0f172a] tracking-tightest flex items-center gap-3">
      ...
      <div className="flex-1 px-2 space-y-8 mt-4 custom-scrollbar">
        {categories.map((cat) => (
          <div key={cat}>
      ...
      <div className="p-6 mt-auto border-t border-slate-50">
        <div className="p-5 glass-card bg-slate-50/50 border-transparent hover:bg-slate-50/80 cursor-pointer transition-all duration-300 flex items-center gap-4">
          <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white font-black text-xs shadow-md shadow-indigo-100 ring-2 ring-white">
            YA
          </div>
          <div className="flex-1 overflow-hidden">
            <p className="text-sm font-black text-slate-900 truncate leading-tight">Young Alaska</p>
            <p className="text-[10px] text-indigo-600 font-bold uppercase tracking-tighter">Business Pro</p>
          </div>
          <ChevronDown size={14} className="text-slate-400 opacity-50" />
        </div>
        <button className="w-full mt-6 py-4 gradient-btn rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl shadow-indigo-100 hover:shadow-indigo-200 active:scale-[0.98]">
          Upgrade to Vision
        </button>
      </div>
    </aside>
