...
const KpiCard = ({ title, value, trend, isPositive }: KpiCardProps) => {
  return (
    <div className="glass-card p-6 bg-white relative group overflow-hidden border-transparent hover:border-indigo-100">
      <div className="absolute top-0 right-0 p-4 opacity-[0.03] group-hover:opacity-[0.08] transition-opacity duration-700">
        <Activity size={60} strokeWidth={3} className="text-indigo-600" />
      </div>
      ...
      <div className="flex items-baseline gap-4 relative z-10">
        <h4 className="text-3xl font-black text-[#0f172a] tracking-tightest leading-none">
          {value}
        </h4>
        <div className={`flex items-center gap-1.5 text-[11px] font-black px-2.5 py-1.5 rounded-lg border ${
          isPositive 
