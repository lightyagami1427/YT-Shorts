...
export default function Dashboard() {
  ...
  return (
    <div className="app-container bg-[#fcfdfe] min-h-screen">
      <Sidebar />
      <main className="main-content flex flex-col gap-8 px-8 py-10 max-w-[1400px] mx-auto">
        
        {/* Top Header */}
        <header className="flex justify-between items-center bg-white/70 backdrop-blur-xl px-6 py-4 rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] sticky top-6 z-50">
          <div className="relative group">
            ...
            <input 
              ...
              className="w-[360px] pl-12 pr-4 py-3 bg-slate-50/50 border border-transparent rounded-2xl text-sm font-bold focus:outline-none focus:bg-white focus:border-indigo-100 focus:ring-4 focus:ring-indigo-50/50 transition-all placeholder:text-slate-500"
            />
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="flex items-center gap-4 group cursor-pointer">
              ...
              <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-500 border-2 border-white shadow-xl shadow-indigo-100 ring-1 ring-slate-100" />
            </div>
          </div>
        </header>
        ...
        <section className="animate-in fade-in duration-700">
          <div className="flex justify-between items-end mb-10">
            <div>
              ...
              <h2 className="text-3xl font-black text-[#0f172a] tracking-tightest">Control Center</h2>
            </div>
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2 bg-white border border-slate-100 p-1.5 rounded-2xl shadow-sm">
                <button 
                  onClick={() => setTimeFilter('24h')}
                  className={`px-4 py-2 text-xs font-black transition-all rounded-xl ${timeFilter === '24h' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-slate-400 hover:text-indigo-600'}`}
                >
                  24h
                </button>
                <button 
                  onClick={() => setTimeFilter('7d')}
                  className={`px-4 py-2 text-xs font-black transition-all rounded-xl ${timeFilter === '7d' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-slate-400 hover:text-indigo-600'}`}
                >
                  7d
                </button>
                <button 
                  onClick={() => setTimeFilter('30d')}
                  className={`px-4 py-2 text-xs font-black transition-all rounded-xl ${timeFilter === '30d' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-slate-400 hover:text-indigo-600'}`}
                >
                  30d
                </button>
              </div>
              <button className="p-3 bg-white border border-slate-100 rounded-2xl text-slate-400 hover:text-indigo-600 hover:bg-slate-50 transition-all shadow-sm">
          ...
          <div className="flex items-center gap-3 overflow-x-auto pb-2 custom-scrollbar">
            {CATEGORIES.map((cat) => (
              <button
                ...
                className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all duration-300 border-2 ${
        ...
        {/* Intelligence Grid */}
        <section className="grid grid-cols-1 xl:grid-cols-12 gap-8 animate-in fade-in slide-in-from-bottom-8 duration-1000">
          <div className="xl:col-span-8 flex flex-col gap-6">
            <div className="glass-card bg-white p-8 border-transparent">
              <div className="flex justify-between items-center mb-8 border-b border-slate-50 pb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-[#0f172a] tracking-tight">Trending Intelligence</h3>
                    <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">
                      {activeCategory === 'all' ? 'Cross-platform US Analysis' : `${CATEGORIES.find(c => c.key === activeCategory)?.label} · US Analysis`}
                    </p>
                  </div>
                </div>
                <button onClick={() => fetchTrends(true)} className="p-3 hover:bg-slate-50 rounded-xl text-slate-400 transition-all border border-slate-50 hover:border-slate-100">
              ...
              <div className="grid grid-cols-1 gap-6">
                {loading ? (
                  ...
                  trends.map((topic, i) => (
                    <div key={i} className="flex items-center justify-between p-6 rounded-[1.5rem] hover:bg-slate-50 transition-all group border-2 border-transparent hover:border-indigo-50/50 cursor-pointer relative overflow-hidden">
                      <div className="flex items-center gap-6 relative z-10">
                        <div className={`w-12 h-12 rounded-[1.25rem] flex items-center justify-center font-black text-lg shadow-sm border-2 ${getSourceColor(topic.source)}`}>
                          {topic.source[0]}
                        </div>
                        <div>
                      ...
                      <Link 
                        ...
                        className="flex items-center gap-3 bg-white border border-slate-100 px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-sm group-hover:bg-indigo-600 group-hover:text-white group-hover:border-indigo-600 group-hover:shadow-indigo-100 transition-all relative z-10 active:scale-95"
                      >
                        Launch Studio
          ...
          <div className="xl:col-span-4 flex flex-col gap-8">
            <div className="rounded-[2rem] bg-[#4f46e5] p-10 text-white relative overflow-hidden group shadow-2xl shadow-indigo-100">
               ...
               <div className="relative z-10">
                  <div className="flex items-center gap-2 mb-6">
                    <div className="w-8 h-8 rounded-xl bg-white/20 flex items-center justify-center backdrop-blur-md border border-white/20">
            ...
            <div className="glass-card bg-white p-8 border-transparent relative overflow-hidden flex flex-col items-center text-center justify-center gap-5 group cursor-pointer hover:bg-slate-50 transition-colors">
               <div className="w-12 h-12 rounded-2xl bg-indigo-50 flex items-center justify-center text-indigo-600 group-hover:scale-110 group-hover:rotate-12 transition-all duration-500">
                  <Sparkles size={20} strokeWidth={2.5} fill="currentColor" />
               </div>
               <div>
                  <h4 className="text-lg font-black text-[#0f172a] mb-1">Automate with AI</h4>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-loose">Launch 10 unique scripts<br/>In less than 60 seconds</p>
               </div>
               <button className="mt-2 px-8 py-3 bg-slate-900 text-white rounded-xl text-[9px] font-black uppercase tracking-[0.2em] shadow-lg shadow-slate-200 hover:shadow-indigo-100 transition-all">
                  Initialize Batch
               </button>
            </div>
          </div>
